...
object FingerTree {
  ...
  final case class Single[+A](a: A) extends FingerTree[A] {
    val headLeft = a
    val tailLeft = Empty
  ...
  final case class Deep[+A](prefix: Digit[A],
...
//            case ViewNilRight() => prefix.toTree   // this match crashes Scalac 2.9.1 -- thus use the wildcard instead
  ...
  final case class Node3[+A](a1: A, a2: A, a3: A)
    extends Node[A] {
  ...
  final case class FTConsLeft[+S[+_], +A](head: A, tail: S[A])
  ...
  final case class FTConsRight[+S[+_], +A](tail: S[A], head: A)
  ...
  final case class Three[+A](a1: A, a2: A, a3: A)
    extends Digit[A] {
  ...
  final case class Four[+A](a1: A, a2: A, a3: A, a4: A)
    extends Digit[A] {
